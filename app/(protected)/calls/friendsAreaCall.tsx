...
import { FriendRequestsType } from "@/lib/types";
import {
  faMinus,
  faPhone,
  faPlus,
...
export default function FriendAreaCalls({ setActiveCall }: Props) {
  ...
  const callMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      router.push(`/calls?callID=${data.id}&role=caller`);
      console.log(data);
    },
  });

  const filteredResults = friends.filter((user) =>
            ...
            {filteredResults.length == 0 ? (
              ...
              <div className=" h-full w-full overflow-y-auto">
                {filteredResults.map((user) => (
                  <div
                    key={user.friend.uid}
                    className="flex p-2 border-b-2 border-gray-400 items-center gap-2 w-full"
                  >
                    ...
                    <div className="ml-auto flex items-center justify-center gap-2 text-gray-700 *:hover:cursor-pointer">
                      <button
                        className=" p-1 flex-1"
                        onClick={() => {
                          callMutation.mutate({
                            receiverID: user.friend.uid,
                            type: "audio",
                          });
                        }}
